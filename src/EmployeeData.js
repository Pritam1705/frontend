...
export function ListAllEmployees() {
  ...
  const FetchData = async () => {
    const data = await fetch('http://localhost:8083/api/v1/employee/search/all');
...
export function ListEmployeeActiveEmployee() {
  ...
  const FetchData = async () => {
    const data = await fetch('http://localhost:8083/api/v1/employee/search/status');
    const stats = await data.json();
    handleStats(stats);
  };
...
export function RoleDistribution() {
  ...
  const FetchData = async () => {
    const data = await fetch('http://localhost:8083/api/v1/employee/search/roles');
    const stats = await data.json();
    handleStats(stats);
  };
...
export function LocationDistribution() {
  ...
  const FetchData = async () => {
    const data = await fetch('http://localhost:8083/api/v1/employee/search/location');
    const stats = await data.json();
    handleStats(stats);
  };
          ...
          <C3Chart
            ...
            data={{
              columns: [
                ...
                ["New York", generateResult(empData["New York"])],
              ],
              type: "donut",
              colors: {
